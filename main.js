...
import { Charge, dt, numberOfParticles } from "./js/physics/charge.js";
...
import { quadTree3D, getNetForce, Point3D, recursion, resetCounterAll, iterations, counter1, dis, determineFields } from "./js/misc/octree.js";
...
/**
 * @param {boolean} oTime - true is o(n log n). false if o(n * n). defaults to true
 * 
 * @param {boolean} electric - if true displays electric fields. defaults to true
 * 
 * @param {boolean} magnetic - if true displays magnetic fields. defaults to true
 * 
 * #-----------------------#
 * 
 * number of vectors drawn = (range/distantBetween)^2
 * 
 * O(n log n * # of vectors) time if o(n log n) time
 * 
 * Or O(# of particles * # of vectors) time if o(n * n) time
*/
function drawFields(oTime = true, magnetic = true, electric = true, range = 2, distantBetweenVectors = 0.4, speed = 0.01) {
    counter2.v += speed;
    if(!magnetic&&!electric) return;
    for (let x = -range; x < range; x += distantBetweenVectors) {
        for (let y = -range; y < range; y += distantBetweenVectors) {
            //for (let z=-2;z<2;z+=0.5){
            let p = new Vector3(x,y,Math.sin(counter2.v));
            if (!oTime) {
                if (magnetic) magneticFieldAt(p, true);
                if (electric) electricFieldAt(p, true);
            }//}
            else {
                let vectors = determineFields(p,starting);
                //if(p.z>0.999&&p.x>1.9&&p.y>1.9) console.log(vectors)
                //if(vectors.e.x==NaN||vectors.m.x==NaN) console.log(vectors)
                if(electric) drawVector(vectors.e,p,"rgb(255, 255, 255)", 2/(numberOfParticles));
                if(magnetic) drawVector(vectors.m,p,"rgb(25, 127, 0)");
            }
        }
    }
}

let drawFieldsVars = {on:false,oTime:true,magnetic:true,electric:true,range:2,distant:0.4,speed:0.01}

function setFieldsVar(on,oTime, magnetic, electric, range, distant, speed){
    drawFieldsVars.on = on;
    drawFieldsVars.oTime=oTime;
    drawFieldsVars.electric=electric;
    drawFieldsVars.speed=speed;
    drawFieldsVars.distant=distant;
    drawFieldsVars.range=range;
    drawFieldsVars.magnetic=magnetic;
    console.log(drawFieldsVars);
}
...
let animateFrame;
function animate() {
    if(pause) {cancelAnimationFrame(animateFrame); return;}
    ...
    if(drawFieldsVars.on) drawFields(drawFieldsVars.oTime,drawFieldsVars.magnetic,drawFieldsVars.electric,drawFieldsVars.range,drawFieldsVars.distant,drawFieldsVars.speed);
...
let interval = setInterval(
    () => {
        ...
        console.log(`Peformace: ${1000/(performaceCheck.check2 - performaceCheck.check1)} FPS`);
        console.log('--------------------');
    },
    5000
);

export {animate, notPause, setFieldsVar};